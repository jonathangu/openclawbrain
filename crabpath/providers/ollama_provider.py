"""Ollama embedding and routing provider implementations."""
...
class OllamaEmbeddingProvider(EmbeddingProvider):
    ...
    def __init__(self, model: str = "nomic-embed-text", base_url: str = "http://localhost:11434") -> None:
        ...
        self._embed_fn = ollama_embed(model=model, base_url=base_url)
        self._dimensions = 0
...
class OllamaRouterProvider(RouterProvider):
    name = "ollama"
    ...
    def __init__(
        ...
        if not base_url:
            raise RuntimeError("base_url is required for Ollama router provider.")
        self.model = model
    ...
    def _call_chat(self, messages: list[dict[str, str]]) -> str:
        ...
        if not content:
            raise RuntimeError(f"Ollama returned no content: {result}")
        return str(content)
