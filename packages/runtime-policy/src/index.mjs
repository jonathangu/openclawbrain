...
export function decideOpenClawBrainIntervention(input) {
  ...
  if (normalized.runtimeMode === "off") return staySilent({ ...proofBase, reason: "runtime mode is off" });
  const requested = selectedPolicyKind(normalized.redactedTurn.turnType);
  const proof = { ...proofBase, reason: selectedPolicyReason(normalized.redactedTurn.turnType, requested), candidate_memory_count: normalized.candidateMemories.length, selected_memory_ids_redacted: selectMemoryIds(normalized) };
  if (normalized.runtimeMode === "proof-only") return { kind: "proof_only", proof: { ...proof, decision: "proof_only", reason: `proof-only mode: ${proof.reason}` } };
  if (requested === "stay_silent") return { kind: "stay_silent", proof: { ...proof, decision: "stay_silent" } };
  if (requested === "correction_only") {
    const message = correctionMessage(normalized);
    if (!message) return staySilent({ ...proof, reason: "correction policy fired but no safe redacted correction candidate was available" });
    return { kind: "correction_only", message, proof: { ...proof, decision: "correction_only" } };
  }
  if (requested === "full_context") {
    const context = fullContextMessage(normalized);
    if (!context) return staySilent({ ...proof, reason: "context policy fired but no safe redacted context candidate was available" });
    return { kind: "full_context", context, proof: { ...proof, decision: "full_context" } };
  }
  return staySilent({ ...proof, reason: "unknown turn type defaults to silence" });
}
...
export function normalizeRuntimePolicyInput(input) {
  ...
  return { profileId: String(input.profileId).trim(), runtimeMode, redactedTurn: normalizeRedactedTurn(input.redactedTurn), candidateMemories: (input.candidateMemories ?? []).map(normalizeMemoryCandidate).filter(Boolean), toolContext: input.toolContext ? normalizeToolContext(input.toolContext) : undefined };
}
...
function normalizeRedactedTurn(turn) { const turnType = classifyRedactedTurn(turn); return { turnId: safeString(turn.turnId ?? turn.id ?? "turn-redacted"), turnType, summary: safeString(turn.summary ?? turn.userMessageRedacted ?? ""), channel: safeString(turn.channel ?? "unknown"), agentId: safeString(turn.agentId ?? turn.profileId ?? "main") }; }
...
function buildProofBase(input) { return { schema: "ocb.proof_event.v1", policy_version: RUNTIME_POLICY_VERSION, profile_id: input.profileId, runtime_mode: input.runtimeMode, turn_id_redacted: input.redactedTurn.turnId, turn_type: input.redactedTurn.turnType, raw_text_stored: false, raw_transcript_upload: false, redacted_proof_event: true, created_at: new Date().toISOString() }; }
function staySilent(proof) { return { kind: "stay_silent", proof: { ...proof, decision: "stay_silent" } }; }
