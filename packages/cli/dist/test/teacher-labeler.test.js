...
import { createOllamaTeacherLabeler, summarizeTeacherLabelerOpportunity } from "../src/teacher-labeler.js";
...
test("teacher labeler opportunity counts session-tail message_delivered interactions with matched decisions", () => {
    const interaction = makeMessageDeliveredInteraction();
    const normalizedEventExport = buildNormalizedEventExport({
        interactionEvents: [interaction],
        feedbackEvents: []
    });
    const summary = summarizeTeacherLabelerOpportunity({
        ...
        client: {
            async generate() {
                throw new Error("opportunity summary should not call the labeler client");
            }
        }
    });
    ...
    assert.equal(summary.budgetedCandidateCount, 1);
    assert.equal(summary.status, "ready");
    assert.equal(summary.detail, "candidates=1");
});

test("teacher labeler can materialize an artifact from a session-tail message_delivered interaction", async () => {
    const interaction = makeMessageDeliveredInteraction();
    const normalizedEventExport = buildNormalizedEventExport({
        interactionEvents: [interaction],
        feedbackEvents: []
