...
function buildGraphBlockIdResolver(graph) {
    const blockIds = new Set(graph.blocks.map((block) => block.id));
    ...
    for (const block of graph.blocks) {
        if (block.compactedFrom !== undefined) {
            for (const originId of block.compactedFrom) {
                if (!compactedFromIndex.has(originId)) {
        ...
        const suffix = block.id.slice(colonIdx + 1);
        if (!suffixIndex.has(suffix)) {
            suffixIndex.set(suffix, block.id);
        }
    }
    return (blockId) => {
        ...
        const compacted = compactedFromIndex.get(blockId);
        if (compacted !== undefined && blockIds.has(compacted)) {
            return compacted;
        }
        ...
        return suffixIndex.get(blockId.slice(colonIdx + 1)) ?? null;
    };
}
function remapServeTimeDecisionToGraph(decision, resolveBlockId) {
    const remapIds = (blockIds) => {
        ...
        for (const blockId of blockIds) {
            ...
            if (resolvedBlockId === null || seen.has(resolvedBlockId)) {
                continue;
            }
            seen.add(resolvedBlockId);
            remapped.push(resolvedBlockId);
        }
        return remapped;
    };
    const remappedChosenContextIds = remapIds(decision.chosenContextIds);
    const remappedCandidateSetIds = remapIds(decision.candidateSetIds);
    const remappedSelectedKernelContextIds = remapIds(decision.selectedKernelContextIds);
    ...
    for (const score of candidateScores) {
        ...
        const existing = remappedScoresByBlockId.get(resolvedBlockId);
        const preferred = existing === undefined || score.actionScore > existing.actionScore
            ? { ...score, blockId: resolvedBlockId, compactedFrom: remapIds(score.compactedFrom) }
            : { ...existing };
        preferred.selected = preferred.selected || score.selected || chosenSet.has(resolvedBlockId);
        preferred.compactedFrom = [...new Set([...(preferred.compactedFrom ?? []), ...remapIds(score.compactedFrom)])];
        preferred.matchedTokens = [...new Set([...(preferred.matchedTokens ?? []), ...(score.matchedTokens ?? [])])];
        preferred.routingChannels = [...new Set([...(preferred.routingChannels ?? []), ...(score.routingChannels ?? [])])];
        remappedScoresByBlockId.set(resolvedBlockId, preferred);
    }
    return {
        ...decision,
        candidateSetIds: remappedCandidateSetIds,
        chosenContextIds: remappedChosenContextIds,
        candidateScores: [...remappedScoresByBlockId.values()]
            .sort((left, right) => right.actionScore - left.actionScore || left.blockId.localeCompare(right.blockId)),
        kernelContextCount: remappedSelectedKernelContextIds.length,
        brainContextCount: remappedSelectedBrainContextIds.length,
        selectedKernelContextIds: remappedSelectedKernelContextIds,
        selectedBrainContextIds: remappedSelectedBrainContextIds
    };
}
/**
...
export function joinDecisionsWithFeedback(decisions, eventExport, maxDelayMs = 300_000) {
    ...
    for (const interaction of eventExport.interactionEvents ?? []) {
        ...
        if (interactionId !== undefined && !interactionById.has(interactionId)) {
    ...
    for (const event of eventExport.feedbackEvents ?? []) {
        ...
        const interaction = interactionById.get(relatedInteractionId);
        ...
        const reward = rewardForEvent(event, undefined);
        if (reward !== 0) {
            const current = outcomes.get(matchedDecision.recordId) ?? 0;
            if (current === 0 || Math.abs(reward) > Math.abs(current)) {
    ...
    for (const decision of decisions) {
        if ((outcomes.get(decision.recordId) ?? 0) !== 0) {
            continue;
        }
...
function createRouterArtifactV2(packId, builtAt, graph, vectors, eventExport, serveTimeDecisions, baselineState, sparseFeedback, principalBacklog, teacherObservationOutcomes = []) {
    ...
    const trajectories = [];
    for (const decision of remappedServeTimeDecisions) {
    ...
    for (const trajectory of trajectories) {
        ...
        const decision = remappedServeTimeDecisions.find((d) => d.recordId === trajectory.turnId ||
