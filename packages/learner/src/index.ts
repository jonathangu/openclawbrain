...
function createGraphPayload(
  ...
  const teacherBlocks = teacherSupervisionBlocks(packId, teacherSupervisionArtifacts);
  const graphSeedBlocks = [...baseBlocks, ...teacherBlocks];
  const metadataById = buildBlockMetadata(packId, workspace, eventExport);
  const evolved = applyGraphEvolution(packId, builtAt, graphSeedBlocks, metadataById, structuralOpsSummary(input), eventExport);
