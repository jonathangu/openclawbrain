...
export function deriveDecision(rows: NormalizedRow[], engineeringE2EComplete = false): DecisionSummary {
  ...
  const harmCount = productRows.filter((row) => row.outcome === "harm" || row.harmFlags.length > 0).length;
  const winCount = productRows.filter((row) => row.outcome === "win").length;
  const harm = rate(harmCount, productRows.length);
  const wins = rate(winCount, productRows.length);
  ...
  const thresholds = {
    ...
    judging_complete_for_product_rows: hasJudgments,
  ...
  const thresholdBasis = {
    ...
    judging_complete_for_product_rows: `${judgedProductRows}/${productRows.length} product rows`,
    harm_rate_at_or_below_5_percent: `${harmCount}/${productRows.length} harms; max ${MAX_HARM_RATE * 100}%`,
    win_rate_at_or_above_60_percent: `${winCount}/${productRows.length} wins; min ${MIN_WIN_RATE * 100}%`,
  };
