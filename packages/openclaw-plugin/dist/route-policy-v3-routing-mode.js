export function detectRoutingModeV3(frame, message = '') {
...
export function hybridWeightsForRoutingModeV3(mode) {
    switch (mode) {
...
export function calibrationThresholdAdjustmentForModeV3(mode) {
    switch (mode) {
        case 'exact_correction':
            return 0.04;
        case 'semantic_planning':
...
export function prototypeRiskPenaltyV3(prototype, mode) {
    let penalty = 0;
    if (prototype.syncPlanner === 'allowed' || prototype.syncPlanner === 'prefer')
        penalty += 0.04;
    if (prototype.graphDepth >= 2)
        penalty += 0.03;
    penalty += Math.min(0.08, Math.max(0, Number(prototype.harmPrior || 0)) * 0.04);
    if (mode === 'casual_silence' && prototype.route !== 'no_memory')
        penalty += 0.08;
    if (mode === 'exact_correction' && !prototype.memoryTypes.includes('correction') && prototype.route !== 'no_memory')
        penalty += 0.05;
    if (mode === 'semantic_planning' && prototype.route === 'no_memory')
        penalty += 0.03;
    return penalty;
}
export function routeModeDiagnosticFamilyV3(mode, route) {
    return `${mode}:${route}`;
}
