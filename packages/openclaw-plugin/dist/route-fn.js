...
function heuristicRoutePlan(packet, turnFrame, config, policySnapshots, retrievalIntent, captureIntent) {
    const message = packet.latestUserMessageRedacted.toLowerCase();
    ...
    const v3PolicyMatch = snapshots.v3
    ...
    const v2PolicyMatch = snapshots.v2
...
function routePlanFromPolicyRule(packet, turnFrame, config, policySnapshot, match, retrievalIntent, captureIntent, routingMode, fallbackSource) {
...
function normalizePolicySnapshotBundle(policySnapshots) {
    if (!policySnapshots)
        return {};
    if (policySnapshots.v3 || policySnapshots.v2 || policySnapshots.legacy)
        return policySnapshots;
    if (policySnapshots.version === 'route-policy-v3')
        return { v3: policySnapshots };
    if (policySnapshots.version === 'route-policy-v2')
        return { v2: policySnapshots };
    return { legacy: policySnapshots };
}
function isDirectPolicyMatch(match, minRouteConfidence, explicitCorrectionCue) {
    if (!match?.matched || !match.rule || Number(match.score ?? 0) < minRouteConfidence)
        return false;
...
function applyPolicySnapshot(packet, turnFrame, policySnapshot) {
    const boost = { route: null, confidence: 0, memoryTypes: [], queries: [], graphDepth: undefined, matchedPolicyRuleId: undefined, reasonCode: undefined, rawPolicyScore: undefined, calibratedPolicyScore: undefined, policyThreshold: undefined, abstained: undefined };
