...
export async function ensureActivationRoot(config, agentId) {
    const root = activationRootForAgent(config, agentId);
    await mkdir(root, { recursive: true, mode: 0o700 });
    await chmodOwnerOnly(root);
    return root;
}
export async function readActivationContext(config, agentId, decision) {
    if (!config.includeActivationContext) {
        return { text: '', usedFileIdsRedacted: [], rejectedFiles: [] };
    }
    const root = activationRootForAgent(config, agentId);
    if (isRemoteUrl(root))
        throw new Error('activationRoot must be local');
    await mkdir(root, { recursive: true, mode: 0o700 });
    await chmodOwnerOnly(root);
    ...
    for (const name of names) {
        ...
        if (!FIXED_CONTEXT_FILES.includes(name) || path.dirname(fullPath) !== root)
            continue;
        try {
...
export function buildInjectionText(decision, activationContext, config) {
    if (decision.kind !== 'correction_only' && decision.kind !== 'full_context')
        return '';
    const parts = [];
    ...
    if (activationContext.text)
        parts.push(activationContext.text);
...
export function filesForDecision(decision) {
    if (decision.kind === 'correction_only')
        return ['corrections.md'];
    if (decision.kind === 'full_context' && decision.slice === 'tool-heavy')
        return ['context.md', 'corrections.md', 'tool-guidance.md'];
    if (decision.kind === 'full_context')
        return ['context.md', 'corrections.md'];
    return [];
}
function fileId(name, text = '') {
    return `activation-file:${name}:${hashText(`${name}\n${text}`).slice(0, 24)}`;
}
async function chmodOwnerOnly(root) {
    try {
