...
function classifyRouteOutcome(decision, injections) {
    if (injections.length === 0) {
        if (decision.selectedMemoryIds.length === 0)
            return { outcome: 'no_signal', reward: 0 };
        return null;
    }
    if (injections.some((injection) => injection.outcome === 'pending'))
        return null;
...
function buildRouteExample(agentId, decision, injections, outcome, reward) {
    return {
        agentId,
...
function buildLesson(decision, injections, outcome) {
...
function applyMemoryUpdates(store, injections) {
    let updated = 0;
...
function scorePatchForOutcome(outcome) {
    switch (outcome) {
        case 'helped':
        case 'accepted':
...
function maybeBuildPolicySnapshot(store, agentId, config) {
    ...
    if (!existing && examples.length === 0)
        return null;
    if (!existing && examples.length < config.learning.minExamplesForPolicyUpdate)
        return null;
