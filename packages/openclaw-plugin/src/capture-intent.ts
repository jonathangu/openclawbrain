...
export interface MemoryScopeHint { kind: MemoryScopeKind; key?: string }
...
export type RetrievalIntent =
  ...
  | 'no_retrieval'
  | 'memory_management'
  | 'recall_value_request';

export type RiskHint =
  | 'ordinary'
  | 'private'
  | 'codeword_like_value'
  | 'credential_like_secret'
  | 'ambiguous_sensitive_recall'
  | 'benign_recall'
  | 'delete_requested';

export type SensitiveValueRisk =
  | { kind: 'ordinary'; plaintextAllowed: true; proactiveInjectionAllowed: true; reason: string }
  | { kind: 'user_authorized_recall'; plaintextAllowed: boolean; proactiveInjectionAllowed: false; reason: string }
  | { kind: 'ambiguous_codeword'; plaintextAllowed: false; proactiveInjectionAllowed: false; reason: string }
  | { kind: 'credential_secret'; plaintextAllowed: false; proactiveInjectionAllowed: false; reason: string };
...
export function detectCaptureIntent(input: { latestUserMessageRedacted: string; agentId?: string; sessionId?: string; sessionKey?: string }): CaptureIntentResult {
...
export function detectRetrievalIntent(input: { latestUserMessageRedacted: string; agentId?: string; sessionId?: string; sessionKey?: string }): RetrievalIntentResult {
  const raw = String(input.latestUserMessageRedacted ?? '').trim();
  const text = normalize(raw);
  const scopeHints = [inferScope(raw, input)].filter(Boolean) as MemoryScopeHint[];
  const query = raw || '';
...
export function classifySensitiveValue(text: string, captureIntent?: CaptureIntent): SensitiveValueRisk {
...
export function captureStoreThreshold(intent: CaptureIntent): number {
  switch (intent) {
...
function result(shouldConsiderCapture: boolean, intent: CaptureIntent, confidence: number, reason: string, matchedSignals: string[], riskHints: RiskHint[], proposedScope?: MemoryScopeHint): CaptureIntentResult {
  return { shouldConsiderCapture, intent, confidence, reason, matchedSignals, riskHints, proposedScope };
}

function retrieval(shouldRetrieve: boolean, intent: RetrievalIntent, confidence: number, query: string, scopeHints: MemoryScopeHint[], includeRecallRules: boolean, memoryTypes: MemoryType[]): RetrievalIntentResult {
...
function matchesExplicitStore(text: string) {
...
function matchesWorkflow(text: string) {
...
function matchesProjectFact(text: string) {
...
function matchesRecallRule(text: string) {
...
function looksOneOff(text: string) {
...
function matchesOutcome(text: string) {
...
function riskHintsForText(text: string, intent: CaptureIntent): RiskHint[] {
  const hints = new Set<RiskHint>();
  if (matchesCredentialSecret(text)) hints.add('credential_like_secret');
  if (/\b(codeword|passphrase|auth phrase|authentication phrase)\b/i.test(text)) hints.add(intent === 'recall_rule' ? 'codeword_like_value' : 'ambiguous_sensitive_recall');
  if (intent === 'recall_rule') hints.add('benign_recall');
  if (hints.size === 0) hints.add('ordinary');
  return [...hints];
}

function inferScope(text: string, input: { agentId?: string; sessionId?: string; sessionKey?: string }): MemoryScopeHint {
  if (/\btelegram\b/i.test(text)) return { kind: 'channel', key: 'telegram' };
  if (/\bcormorantai\b/i.test(text)) return { kind: 'app', key: 'CormorantAI' };
  if (/\bopenclawbrain|openclaw brain|ocb\b/i.test(text)) return { kind: 'repo', key: 'openclawbrain' };
  if (/\bpelican\b/i.test(text)) return { kind: 'project', key: 'Pelican' };
  if (/\bbountiful\b/i.test(text)) return { kind: 'project', key: 'Bountiful Garden' };
  const forProject = text.match(/\bfor\s+([A-Z][A-Za-z0-9_-]{2,})\b/);
  if (forProject) return { kind: 'project', key: forProject[1] };
  if (/\bthis repo\b/i.test(text)) return { kind: 'repo', key: 'current_repo' };
  if (/\bthis project\b/i.test(text)) return { kind: 'project', key: 'current_project' };
  if (input.sessionId || input.sessionKey) return { kind: 'agent', key: input.agentId || 'main' };
