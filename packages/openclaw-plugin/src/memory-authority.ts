import type {
  MemoryAuthorityDecisionKind,
  MemoryAuthorityEventType,
  MemoryAuthorityResolution,
  MemoryBehavioralAvailability,
  MemoryNode,
  MemoryPrivacyClass,
  MemoryTemporalValidity,
  MemoryValidationStrategy,
  MemoryValidity,
...
export class MemoryAuthorityResolver {
  ...
  resolveOne(memory: MemoryNode, packet: TurnEventPacket, plan: RoutePlan): MemoryAuthorityResolution {
    ...
    if (temporal.state === 'expired') {
      const nextDecision: MemoryAuthorityDecisionKind = material && validationStrategy === 'user_confirm' ? 'confirm_before_use' : 'abstain';
...
export function defaultValidityForMemory(memory: MemoryNode, overrides: Partial<MemoryValidity> = {}): MemoryValidity {
  ...
  const behavioralAvailability: MemoryBehavioralAvailability = overrides.behavioralAvailability
...
export function authorityEventTypeForDecision(decisionKind: MemoryAuthorityDecisionKind): MemoryAuthorityEventType {
  switch (decisionKind) {
    case 'inject':
      return 'used';
    case 'weak_context':
...
function decision(
  memory: MemoryNode,
  decisionKind: MemoryAuthorityDecisionKind,
  ...
  risk: MemoryAuthorityResolution['risk'],
): MemoryAuthorityResolution {
  return {
    memoryId: memory.id,
...
function temporalState(memory: MemoryNode, validity: MemoryValidity, age: number) {
  const reasons: string[] = [];
  if (validity.expiresAt && Date.parse(validity.expiresAt) <= Date.now()) {
    return { state: 'expired' as MemoryTemporalValidity, multiplier: 0.05, reasons: ['temporal:expires_at_elapsed'] };
  }
  if (validity.validUntil && Date.parse(validity.validUntil) <= Date.now()) {
    return { state: 'expired' as MemoryTemporalValidity, multiplier: 0.05, reasons: ['temporal:valid_until_elapsed'] };
  }
  if (validity.revalidateAfter && Date.parse(validity.revalidateAfter) <= Date.now()) {
    return { state: 'stale' as MemoryTemporalValidity, multiplier: 0.6, reasons: ['temporal:revalidate_after_elapsed'] };
  }
  ...
  if (Number.isFinite(halfLife) && halfLife > 0 && age > halfLife * 4) {
    return { state: 'expired' as MemoryTemporalValidity, multiplier: 0.1, reasons: ['temporal:age_expired'] };
  }
  if (Number.isFinite(halfLife) && halfLife > 0 && age > halfLife) {
    reasons.push('temporal:age_stale');
    return { state: 'stale' as MemoryTemporalValidity, multiplier: Math.max(0.35, Math.pow(0.5, age / halfLife)), reasons };
  }
  if (validity.temporalValidity === 'stale') return { state: 'stale' as MemoryTemporalValidity, multiplier: 0.65, reasons: ['temporal:stored_stale'] };
  if (validity.temporalValidity === 'expired') return { state: 'expired' as MemoryTemporalValidity, multiplier: 0.1, reasons: ['temporal:stored_expired'] };
  return { state: validity.temporalValidity, multiplier: 1, reasons };
}

function estimateRelevance(memory: MemoryNode, lowerMessage: string, plan: RoutePlan) {
  let score = memory.importance * 0.35 + memory.confidence * 0.35 + memory.freshness * 0.2;
  if (plan.retrievalPlan.memoryTypes.includes(memory.type)) score += 0.15;
...
function isMaterial(memory: MemoryNode, lowerMessage: string, relevanceScore: number, risk: MemoryAuthorityResolution['risk']) {
  if (risk === 'high') return true;
  if (relevanceScore >= 0.72) return true;
  if (['correction', 'workflow', 'tool_convention', 'routing_rule', 'agent_assignment'].includes(memory.type)) return true;
  if (/\b(build|test|install|deploy|delete|publish|release|token|secret|password)\b/.test(lowerMessage)) return true;
  return false;
}

function currentInstructionOverrides(memory: MemoryNode, lowerMessage: string): { reason: string; weak?: boolean } | null {
...
function riskForMemory(memory: MemoryNode, validity: MemoryValidity): MemoryAuthorityResolution['risk'] {
  if (memory.type === 'recall_rule' || validity.privacyClass !== 'normal') return 'high';
  if (['routing_rule', 'agent_assignment', 'tool_convention', 'workflow', 'correction'].includes(memory.type)) return 'medium';
  return 'low';
}

function validationStrategyForMemory(memory: MemoryNode, validity: MemoryValidity): MemoryValidationStrategy {
  if (validity.validationStrategy && validity.validationStrategy !== 'none') return validity.validationStrategy;
  return defaultValidationStrategy(memory, validity.privacyClass);
}

function defaultValidationStrategy(memory: MemoryNode, privacyClass: MemoryPrivacyClass): MemoryValidationStrategy {
  if (memory.type === 'recall_rule' || privacyClass === 'recall_only' || privacyClass === 'do_not_reveal_proactively') return 'never_proactive';
  if (['workflow', 'tool_convention', 'project_fact'].includes(memory.type)) return 'environment_check';
  if (['correction', 'preference', 'routing_rule', 'agent_assignment'].includes(memory.type)) return 'user_confirm';
  return 'none';
}

function defaultPrivacyClass(memory: MemoryNode, override?: MemoryPrivacyClass): MemoryPrivacyClass {
  if (override) return override;
  if (memory.type === 'recall_rule') return 'recall_only';
  const text = `${memory.content} ${memory.positive ?? ''}`.toLowerCase();
  if (/\b(password|token|secret|api key|private key|codeword|passphrase)\b/.test(text)) return 'sensitive';
  return 'normal';
}

function defaultDecayPolicy(memory: MemoryNode) {
  if (memory.type === 'recall_rule') return 'sensitive_no_decay';
  if (['workflow', 'tool_convention', 'project_fact'].includes(memory.type)) return 'environment_verified';
  if (memory.type === 'preference') return 'soft_preference';
  if (memory.type === 'correction') return 'correction_lineage';
  return 'standard';
}

function defaultHalfLifeDays(memory: MemoryNode) {
  switch (memory.type) {
    case 'workflow':
...
function scopeSpecificity(memory: MemoryNode) {
