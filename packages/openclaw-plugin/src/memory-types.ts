...
export type MemoryAuthorityDecisionKind =
  | 'inject'
  | 'weak_context'
  | 'verify_before_use'
  | 'confirm_before_use'
...
export type GraphMaintenanceProposalStatus =
  | 'drafted'
  | 'guard_rejected'
  | 'pending_review'
  | 'approved'
  ...
  | 'failed_apply'
  | 'stale';
export type GraphMaintenanceProposalType =
  | 'merge_exact_duplicate_nodes'
  | 'retire_bad_edge'
  | 'mark_stale_high_authority'
  | 'block_tombstone_recapture'
  | 'propose_scoped_exception'
  | 'record_feedback_edge_observation';
