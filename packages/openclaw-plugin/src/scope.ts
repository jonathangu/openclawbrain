...
export function scopeContextFromPacket(packet: Partial<TurnEventPacket> = {}): ScopeContext {
  const metadata: any = packet.metadata || {};
...
function inferRepo(text: string) {
  if (/\bopenclawbrain|openclaw brain|ocb\b/i.test(text)) return 'openclawbrain';
  return '';
}

function inferProject(text: string) {
  if (/\bpelican\b/i.test(text)) return 'Pelican';
  if (/\bbountiful\b/i.test(text)) return 'Bountiful Garden';
  return '';
}

function inferApp(text: string) {
  if (/\bcormorantai\b/i.test(text)) return 'CormorantAI';
  return '';
}

export function defaultScopeContext(agentId = 'main'): ScopeContext {
  return { agentId: safeString(agentId || 'main') || 'main' };
}

export function memoryInScope(memory: MemoryNode, context: ScopeContext): boolean {
  if (memory.agentId !== context.agentId) return false;
...
export function filterMemoriesForScope(memories: MemoryNode[], context?: ScopeContext): MemoryNode[] {
