...
import type { FeedbackDistillation, MemoryCandidate } from './memory-types.js';
...
export class FeedbackDistiller {
  ...
  async distill(packet: TurnEventPacket): Promise<{ output: FeedbackDistillation; audit: any; rawOutput: unknown }> {
...
function explicitCorrectionFallback(packet: TurnEventPacket): FeedbackDistillation {
  ...
  if (!correctionCue) return emptyDistillation('no_durable_signal');
  ...
  const distilledText = useInstead
    ...
    : rememberCorrection
      ...
      : '';
  if (!distilledText) return emptyDistillation('fallback_unable_to_extract_correction');
...
function emptyDistillation(modelReasonCode: string): FeedbackDistillation {
  return {
    version: 1,
    shouldStore: false,
    confidence: 0,
