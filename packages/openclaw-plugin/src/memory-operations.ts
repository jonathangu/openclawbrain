...
export class MemoryOperationApplier {
  ...
  applyDistillation(distillation: FeedbackDistillation, packet: TurnEventPacket, context: { captureIntent?: CaptureIntentResult } = {}): ApplyFeedbackResult {
    ...
    this.store.transaction(() => {
      ...
      for (const feedback of distillation.injectionFeedback) {
        ...
        if (changed > 0) resolvedInjections += changed;
        else rejectionReasons.push('scope_mismatch_or_missing_injection');
      }
  ...
  private canonicalExistingMemory(candidate: MemoryCandidate, packet: TurnEventPacket): MemoryNode | null {
    let existing = this.store.findMemoryByNormalizedKey(
      packet.agentId,
  ...
  private createMemory(candidate: MemoryCandidate, packet: TurnEventPacket, normalizedKey = candidate.normalizedKey): MemoryNode {
  ...
  private isBlockedByTombstone(candidate: MemoryCandidate, packet: TurnEventPacket) {
    const existing = this.store.findMemoryByNormalizedKeyAny(
      packet.agentId,
      candidate.normalizedKey,
      candidate.scope.kind,
      candidate.scope.key,
...
function sameMemoryContent(existing: MemoryNode, candidate: MemoryCandidate) {
...
function revisionKey(normalizedKey: string, content: string) {
  return `${normalizedKey}:rev:${shortHash(content)}`;
}

function shouldCreateTombstone(text: string) {
...
function shouldRedactTombstone(requestText: string, memoryText: string) {
  return /\b(codeword|passphrase|secret|password|token|api key|private key)\b/i.test(`${requestText} ${memoryText}`);
}

function deletionQueryMatchesMemory(query: string, memory: MemoryNode) {
