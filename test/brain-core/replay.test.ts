...
describe("replay", () => {
  ...
  it("ignores legacy forced-stop reasons even when stopTruth is missing", () => {
    const graph = new BrainGraph();
    graph.addNode(makeNode("a"));
    graph.addNode(makeNode("b"));
    graph.addNode(makeNode("c"));
    graph.addEdge(makeEdge("a", "b", 2));
    graph.addEdge(makeEdge("a", "c", 1.5));

    const forcedStopEpisode: Episode = {
      id: "ep-forced-stop-legacy",
      conversationId: 1,
      queryText: "test",
      queryEmbedding: new Float32Array([1, 0, 0]),
      trajectory: [{
  ...
  it("keeps chosen stop_local distinct from forced stops during replay", () => {
    const graph = new BrainGraph();
    graph.addNode(makeNode("a"));
    graph.addNode(makeNode("b"));
    graph.addNode(makeNode("c"));
    graph.addEdge(makeEdge("a", "b", 2));
    graph.addEdge(makeEdge("a", "c", 1.2));

    const chosenStopEpisode: Episode = {
      id: "ep-chosen-stop",
      conversationId: 1,
      queryText: "test",
      queryEmbedding: new Float32Array([1, 0, 0]),
        ...
        substeps: [
          {
            ...
            candidates: [
              { action: { type: "traverse", targetNodeId: "b" }, score: 0.1, probability: 0.1 },
              { action: { type: "traverse", targetNodeId: "c" }, score: 0.1, probability: 0.1 },
              { action: { type: "stop_local" }, score: 1.2, probability: 0.8 },
            ],
            chosenAction: { type: "stop_local" },
    ...
    const replay = replayEpisode(chosenStopEpisode, graph);
    expect(replay.wouldChange).toBe(true);
    expect(replay.firedNodes).toEqual([]);
  });
});
