...
OpenClawBrain `0.2.33` is a native OpenClaw plugin that keeps a **local SQLite memory graph**, learns from outcomes, injects only bounded context back into the prompt, exposes a quiet Codex continuity bridge, and maintains graph health without patching OpenClaw core.
