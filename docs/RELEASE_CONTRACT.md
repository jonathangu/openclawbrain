...
- **Host-surface validation harness**
  ...
  - Truth: recurrent routing, shadow mode, and current host checks run inside a dedicated sterile validation lane with per-run diagnostic artifacts; deterministic session-bound `brain_teach` proof now exists; plugin registration now has a hook-based compatibility bridge for hosts where `api.registerContextEngine` is gone; and the sterile harness no longer writes the dead `plugins.slots.contextEngine` slot. The raw host lane is still incomplete because the short-static + final narrow worker-down host claims have not yet been rerun/frozen on that repaired seam.
