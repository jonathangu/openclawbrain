...
- Structured evidence harvesting now exists end to end (raw evidence → resolved labels with explicit episode attribution), and harvested completions can now persist multiple concurrent raw signals with extractor metadata before worker resolution; the remaining gap is that source detection still leans on heuristics more than the intended richer human/self/scanner evidence flow.
